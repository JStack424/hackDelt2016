...
$text_input = $_POST['text'];
 ...
 $input = rawurlencode($text_input); //text entry to query
 ...
 $getPressField ='?q=' .$input . '%20-filter:media+from:'.$newsAcc[0].'+OR+from:'. $newsAcc[1].'+OR+from:'. $newsAcc[2] .'+OR+from:'. $newsAcc[3] . '+OR+from:'. $newsAcc[4] .'+OR+from:'. $newsAcc[5] .'+OR+from:'. $newsAcc[6] .'+OR+from:'. $newsAcc[7] .'+OR+from:'. $newsAcc[8] .'+OR+from:'. $newsAcc[9] .'&result_type=mixed';
 $getPopularField = '?q=from%20' . $input . '%20-filter:images&result_type=mixed';
...
$arrayOfArrays = array($result1,$result2,$result3, $text_input);
